...
func TestTupleForwardEmitted(t *testing.T) {
	...
	t.Logf("tuple_forward in %d cases; matrix cells: %v", tagged, cells)
}
...
// TestTupleForwardGeneratedRuns: one GENERATED tuple-forward case builds
// and runs end to end (the handcrafted matrix pins shapes; this pins that
// the emitter's real composition — pair, call site, observation — does).
func TestTupleForwardGeneratedRuns(t *testing.T) {
	if testing.Short() {
		t.Skip("builds and runs a binary")
	}
	for seed := int64(84000); seed < 84100; seed++ {
		cfg := DefaultConfig(seed)
		cfg.Include = []string{"tuple_forward", "helpers", "interfaces", "assertion"}
		c, err := New(cfg).Generate()
		if err != nil {
