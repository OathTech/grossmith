...
| `defer` / `recover` | partial | `defer`, `recover` | Today: obs-event print defer + statement-level guarded catch — gc profile only (excluded for GoLean, no obs* model there). Rung 1 (R1) adds the pure-Go recover-into-named-result wrapper, dissolving the exclusion (sx: g05, g18, g28, c37 progressively). `panic(v)` explicit deferred(panic-identity encoding; L2). |
