...
|---|---|---|---|
| Integer kinds (int, int8..64, uint, uint8..64) | supported | `ints`, `widths`, `width_dependent`; 386 typecheck witness | Width-preserving observation (`goType`) on the GC channel; cross-arch quotient declared via tag — CAVEAT (hunt F10): width_dependent saturates ~98% of programs, so in-tag/off-tag is coverage, not specificity, pending a precision rung. CAVEAT (hunt F15): GoLean's observation channel collapses integer kinds and defined-type identity (structs/interfaces excepted) — kind-defaulting bugs that preserve the numeric value are invisible THERE; the gc-vs-gc channel keeps full width. |
...
| Evaluation order (witnessed) | partial | `order_witness` | R2b mechanism 1 (witness arc W2; effect-discipline design §3): a package accumulator `var wOrd int` plus the sole DESIGNATED IMPURE HELPER `wit(x, tag int) int { wOrd = wOrd*31 + tag; return x }` — E4's amendment covers exactly this pair; every other helper stays pure. Wrapping fires ONLY inside the `order` corner (fourth named corner, 1-in-8 swarm class weight — instruments are minorities, the corner IS the minority mechanism) at the evaluation-order-rich sites: call arguments (spec-ordered left-to-right), multi-assign right sides and INDEX operands (phase-1 evaluation vs phase-2 stores), and comparison/equality operands, which compose with `&&`/`||` into short-circuit operands (whether the right witness fires observes the short-circuit; lands in GoLean's frontend quarantine — expected, classified). The accumulator returns in a trailing observed int slot after the aggregates; under the recover wrapper the defer SNAPSHOTS it, so a mid-statement panic reports the order-before-panic prefix (E3 composition, witnessed). Text-iff-use: `wOrd`/`wit` and the slot exist iff a wrap fired. Wraps under `&&`/`||` carry the `witness_shortcircuit` info tag (the quarantined population — judged-coverage stratifier, review finding 8). CORNER COLLATERAL (review finding 6, measured): inside the order corner `early_return` is fully masked (the arity rule), which also removes ~86% of `dead_code` (earlyReturn is its main source) and the arm re-weighting shifts `break`/`continue`/`assertion` down 16-22%; at the swarm's 1-in-8 corner rate that is ~12% corpus-wide dilution, total for forced-corner runs. Restoring early_return via a per-seed slot reservation is a recorded alternative (it would trade text-iff-use for arity-at-declare). v1 wraps plain `int` operands only — per-type helpers are the recorded extension (both discard paths, unSelf and nonConstExpr, carry the dependency comment — review finding 12); pointer-parameter witnessing (mechanism 2), closures, and general package vars remain deferred below. |
| Pointers, closures, package vars | deferred(effect discipline) | — | The discipline is designed and signed off (2026-08-08); entry is now per-mechanism. Mechanism 1 (`order_witness`, above) is the only entrant; pointers/closures/general package vars await mechanism 2's design note. (sx: g29, c45, c52, g16 partially.) |
...
| `if` / bounded `for` / `range` | supported | `if`, `loops`, `range`, `control_flow` | Literal loop bounds (HALTS). ELEMENT-INCLUSIVE range folds (W3, survey F14): a range over an int-element slice/array may fold `base[i]` alongside the index (info tag `element_fold`) — in range by construction (ranged slices cannot grow by the standing append mask and nothing shrinks one), decided at the fold expression with NO tuple-arity change (the charter's structural rule), conversion form gated+marked conversions/index and kinds-corner-masked, plus a conversion-free plain-int form that survives the kinds corner (in-kind observation). This is what makes shared-backing writes (slice_triple aliases) reachable by observation — index-only folds were element-blind. Range-over-int deferred(language-version parity check first) (sx: c30). |
...
| Helpers and calls | supported | `helpers`, `bare_call`, `functions`, `tuple_forward` | Pure, acyclic, top-level; results assigned or discarded bare (BUG-012's shape). TUPLE FORWARDING (`tuple_forward`, witness arc W0; the 2026-08-08 review §1 / GoLean BUG-049 habitat): `v += sink(src(args))` statements where a 2-3-result source helper call is the sink's COMPLETE argument list, the sink's signature GENERATED to match the tuple across the form matrix — destinations fixed/variadic/mixed × parameter types concrete/any/mixed (pure-variadic×mixed is unrepresentable: one element type). Any-slots are implicit interface boxing at the call (the BUG-049 composition); boxed slots may additionally draw a SOURCE-boxed variant (source result typed `any`, boxing at the source's return) — the conversion-free negative-control rows, `(any, any) -> (any, any)` included, which separate a boxing regression from a generic-forwarding one (mid-arc review finding 2). Assertions read the slots back and are guaranteed-success BY CONSTRUCTION: every sink call has the forwarding SHAPE `tgN(tfN(...))` with matching index (witnessed per call, incl. the index pairing; a cached sink may have several such sites — review finding 4 corrected the earlier single-call-site overstatement), so the dynamic types and the variadic tail arity are generator-known — sinks stay panic-free and their constant `xs[i]` tail indices in range. Sink assertions are can't-fail and carry the `any_slot` info tag so judged-coverage stratification can separate them from the sharp empty-interface assertions (review finding 7). Variadic PARAMETERS exist only in this minimal sink form (typed `...T` and `...any`, folded by constant index — chosen over a range fold to keep the sink draw-free and total); general variadic calls/spread stay deferred(rung; sx: g24, c55). Sink folds gate+mark the constructs they emit (any-slots: interfaces+assertion; variadic tail: slices+index; sized-kind folds: conversions, masked under the kinds corner; string folds: len+strings — audit F1's rule), and the `*31` chain is width_dependent-tagged. Recursion deferred(fuel design), method values/expressions deferred(rung; sx: g16, c53). |
...
- **Rung-blocked, later arcs** (27; the strings family — g09 + the
  ...
  c55 — the minimal variadic-PARAMETER sink form landed with W0's
  `tuple_forward`; general variadic calls/spread remain here), method
  values (g16, c53), nil-interface
